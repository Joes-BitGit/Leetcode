...
class Solution:
    '''
    Time: O(N), N is the lne of the array
    Space: O(1)
    '''
    ...
    def maxSubarraySumCircular(self, A: List[int]) -> int:
        ...
        for i in A:
            ...
            # max ending at that position
            maxSum = max(maxSum, currMax)
            # min so far
            currMin = min(currMin+i, i)
            # min ending at this position
            minSum = min(minSum, currMin)
            # sum of the entire array
            total += i

        # if sum is 0 or negative just return the largest number
        if maxSum <= 0:
            return maxSum
        else:
            # either the normal kadanes algo is the largest sum
            # or the max is spread over the two arrays
